:::options
:title: Use instance fields and properties from `{called_name}` in `{calling_name}` application
:description: This article provides an overview of using instance fields and properties from `{called_name}` in `{calling_name}` application.
...
# Use instance fields and properties from `{called_name}` in `{calling_name}` app  
