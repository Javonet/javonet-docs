:::options
:title: Pass arguments by reference with out keyword to `{called_name}` in `{calling_name}` application.
:description: This article provides an overview of passing arguments by reference with out keyword to `{called_name}` in `{calling_name}` application.
...
# Pass arguments by reference with out keyword to `{called_name}` in `{calling_name}` app
...
## Custom `{calling_name}` with out keyword
