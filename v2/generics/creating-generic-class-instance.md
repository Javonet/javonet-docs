:::options
:title: Create generic `{called_name}` class instance in `{calling_name}` application
:description: This article provides an overview of creating generic `{called_name}` class instance in `{calling_name}` application
...
# Create generic `{called_name}` class instance in `{calling_name}` app
...
## Framework generic class instances in `{calling_name}`
